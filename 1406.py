...
END = pygame.transform.scale(
    pygame.image.load(os.path.join("art", "other", "good.png")), (WIDTH, HEIGHT)
)
...
class Room1:
    def __init__(self):
        ...
        self.phone_call = AudioClue(
            ...
            ROOM_1_DOOR_KNOCK,
            self.mother.fade_in,
            fourth_sound=ROOM_1_CRYING,
        )
        self.tissues = Clickable(self, 361, 317, ROOM_1_TISSUES)
        self.mother_speech = AudioClue(
            self, self.tissues, ROOM_1_MOTHER_SPEAK, 100, False
        )
        self.key = DraggableClue(self, ROOM_1_KEY, 380, 385, True, area=self.drawer)
        self.text = Text(
            "She seems too upset to speak.                                                                 Maybe I should give her some tissues and she'll be able to tell us about the case?",
            "top",
        )
        ...
        self.end_items = [
            ...
            self.tissues,
            self.folder,
            self.folder.next_room_button,
        ]
        self.current_items = [self.key, self.phone, self.mother, self.tissues]
        self.all_items = list(
            set(self.end_items + self.start_items + self.current_items)
        )
    ...
    def play_room(self):
        ...
        if self.phone.clicked == True and self.tissues.clicked == True:
            self.text = Text(
                "Oh good, that seems to have helped her.                                                 Ah! There's the key to the drawer! Let's get that casefile.",
                "top",
            )
            self.key.draw()
        ...
        if self.folder.clicked == True:
            ...
            self.text = Text(
                "Time to go and see what clues we can find in her daughter's bedroom...",
                "top",
            )
...
class Room2:
    def __init__(self):
        ...
        self.twinkle = DraggableClue(
            self, ROOM_2_TWINKLE, 640, 552, True, area=self.diary
        )
        ...
        self.text = Text(
            "Okay, where to look first? Hmm, those posters seems quite interesting. Oh! I've heard of magic ink before! You hold the note over heat and the ink becomes visible!",
            "top",
        )
    ...
    def play_room(self):
        ...
        if self.twinkle.clicked == True:
            ...
            self.text = Text(
                "Aha, that looks like a diary key! Let's grab it and see what we can find out.",
                "top",
            )
...
class Room3:
    def __init__(self):
        ...
        self.peoplespeak = AudioClue(
            self, self.people, ROOM_3_DAD, 1500, False, ROOM_3_MUM
        )
        ...
        self.text = Text(
            "It seems from what she wrote in her diary her friend might know something... Let's see if she'll help.",
            "top",
        )
    ...
    def play_room(self):
        ...
        if self.peoplespeak.sound == "":
            self.text = Text(
                "Hmm, well they won't be much help. What did the daughter shove in the drawer just before I came in? It's locked, maybe the key card is around here...",
                "top",
            )
        ...
        if self.photo.rect.topleft == (self.photo.next_x, self.photo.next_y):
            ...
            self.text_non_italic = Text_non_italic(
                '"So are you sure you don\'t know anything at all?"                                "Then who\'s this boy with you in the photo?"                                        "And what\'s that file your father has?"',
                "bottom",
            )
        ...
        if self.file.rect.topleft == (self.file.next_x, self.file.next_y):
            self.file.collect()
            self.text = Text(
                "It looks like the father collected information on this boy, Tom, he must have been concerned too! Let's see if we can find that boy at that alleyway Emma mentioned.",
                "top",
            )
...
class Room4:
    def __init__(self):
        ...
        self.necklace = CollectableClue(
            self, 600, 355, ROOM_4_NECKLACE, 15, HEIGHT - 105
        )
        self.text = Text(
            "Well, Tom doesn't seem to be anywhere around here.                                I wonder if this man has seen him...",
            "top",
        )
        ...
        self.end_items = [
            ...
            self.bag,
            self.necklace,
            self.necklace.next_room_button,
        ]
        self.all_items = list(set(self.end_items + self.start_items))
        self.has_collectable = True
        self.next_room = False
    ...
    def play_room(self):
        ...
        if self.bottle.image == self.full_bottle and not self.bottle.rect.colliderect(
            self.water
        ):
        ...
        if (
            self.bottle.rect.colliderect(self.drunk_head)
            and self.bottle.image == self.full_bottle
        ):
        ...
        if (
            self.bag.clicked == True
            and self.bottle.image == self.full_bottle
            and self.drunk_talk2.sound == ""
        ):
        ...
        if self.bag.image == self.open_bag and self.drunk_talk2.sound == "":
            ...
            self.necklace.draw()
            self.text = Text(
                "This looks like a lady's necklace, could it be the one Tom gave to Sara? And on the back there's an address. Why would he try to get rid of it, maybe they had a fight?",
                "top",
            )
        ...
        if self.necklace.clicked == True and self.drunk_talk2.sound == "":
            ...
            self.text = Text(
                "Let's go check out that address and see if he will be there.", "top"
            )
...
class Room5:
    def __init__(self):
        ...
        self.car = AudioClue(
            self, self.keyhole, ROOM_5_CAR2, 0, True, ROOM_5_CAR, func=self.man.fade_in
        )
        self.smash = AudioClue(self, self.lock, ROOM_5_SMASH, 50, True)
        self.poster = CollectableClue(
            self, 350, 5, ROOM_5_NOTE, 25, HEIGHT - 115, size=(55, 80)
        )
        self.lock_space = pygame.draw.rect(WIN, TRANSPARENT, (238, 200, 30, 30))
        self.text = Text(
            "Looks like there's no one home. Nothing wrong with having a little look around though. Might be worth trying to get some clues.",
            "top",
        )
    ...
    def play_room(self):
        ...
        if self.man_speak.sound == "":
            self.text = Text(
                "Looks like I'll need some way of breaking in to the garage! That lock doesn't look sturdy, with a little force it should open.",
                "top",
            )
        ...
        if self.poster.self_vis == True:
            self.text.remove_text()
            self.text_italic = Text_non_italic(
                '"Hmm, poster advertising a club... And today the members are meeting up. Guess I am gonna go clubbing tonight..."',
                "bottom",
            )
...
class Room6:
    def __init__(self):
        ...
        self.vip_man_speak = AudioClue(
            self, self.vip_man, ROOM_6_VIP_MAN_SPEAK, 0, True
        )
        self.gang_speak = AudioClue(self, self.gang, ROOM_6_GANG_SPEAK, 0, True)
        self.text_non_italic = Text_non_italic(
            '"Hey, the bouncer won\'t let me through, right? Those guys are my mates from the club. Do you have a spare VIP ticket by any chance?"',
            "bottom",
        )
        self.text = Text("", "bottom")
        ...
        self.end_items = [
            ...
            self.gang,
            self.bat_end,
            self.next_room_button,
        ]
        self.current_items = [self.vip_man, self.window, self.bat, self.gang, self.rope]
        self.all_items = list(
            set(self.end_items + self.start_items + self.current_items + [self.vip])
        )
        self.has_collectable = False
        self.next_room = False
    ...
    def play_room(self):
        ...
        if self.vip_man.clicked == True:
            ...
            self.text_non_italic = Text_non_italic(
                '"Ok, guess I will be spending some money tonight..."', "bottom"
            )
        ...
        if self.rope.clicked == True and self.bouncer.image == "":
            self.rope.image = ""
            self.text_non_italic = Text_non_italic(
                '"Excuse me, I\'m looking for a guy named Tom. Have you seen him?"',
                "bottom",
            )

        if (
            self.gang.rect.collidepoint(pos)
            and pygame.mouse.get_pressed()[0]
            and self.bouncer_vip.image == ""
            and self.rope.image == ""
        ):
        ...
        if self.bat.clicked == True and self.vip.image == "":
            self.text_non_italic = Text_non_italic(
                '"Uh-oh, looks like I better get out of here and fast!"', "bottom"
            )
            self.bat.image = ROOM_6_BAT_TURN
...
class Room7:
    def __init__(self):
        ...
        self.necklace_text_non_italic = Text_non_italic(
            '"Tom gave her a necklace with his address on it, who knows why he\'d want her to go there, seems like he had a rough family dynamic. But why would he have it, just to throw it away?"',
            "bottom",
        )
        self.file_text_non_italic = Text_non_italic(
            "\"Her friend's father was gathering information on Tom which is a bit much, but seems like he didn't actually find a lot to be worried about, he's just a kid.\"",
            "bottom",
        )
        self.photo_text_non_italic = Text_non_italic(
            '"So, we know she was friends with Tom, but all the parents thought he was trouble."',
            "bottom",
        )
        self.poster_text_non_italic = Text_non_italic(
            "\"We didn't find him at the club with his gang, which could be a good sign. But it seems he ran with a rough crowd, I wouldn't trust them.\"",
            "bottom",
        )
        self.diary_text_non_italic = Text_non_italic(
            '"We know from her diary her mother was overprotective of her and would do anything to keep her away from Tom."',
            "bottom",
        )
        self.final_text_non_italic = Text_non_italic(
            '"Now that I\'m looking at all of this, I think I have a good idea of who might know where Sara is!"',
            "bottom",
        )
        self.text = Text("", "bottom")
        ...
        if final:
            ...
            if self.folder.clicked == True:
                ...
                if pygame.mouse.get_pressed()[0] and (
                    self.guess1.rect.collidepoint(pos)
                    or self.guess2.rect.collidepoint(pos)
                    or self.guess4.rect.collidepoint(pos)
                ):
                    self.state = "you_lose"
...
class Room8:
    def __init__(self):
        ...
        self.stick1 = DraggableClue(
            self, ROOM_8_STICK1, 380, 503, True, area=self.plank1
        )
        self.stick2 = DraggableClue(
            self, ROOM_8_STICK2, 797, 490, True, area=self.plank2
        )
        self.stick3 = DraggableClue(
            self, ROOM_8_STICK3, 586, 560, True, area=self.plank3
        )
        ...
        self.mother = FadeIn(
            self, 133, 199, ROOM_8_MOTHER, self.mother_items, self.mother_hit_items
        )
        self.plank_hit = DraggableClue(
            self, ROOM_8_PLANK2, 506, 482, True, area=self.mother
        )
        self.lock_sound = AudioClue(
            self, self.lock, ROOM_8_KEY_LOCK, 500, False, func=self.girl.fade_in
        )
        self.girl_speak = AudioClue(
            self,
            self.girl,
            ROOM_8_GIRL_SPEAK,
            3000,
            False,
            func=self.mother.fade_in,
            func_pause=True,
        )
        self.mother_speak = AudioClue(
            self, self.mother, ROOM_8_MOTHER_SPEAK, 500, False
        )
        self.plank_hit_mother = AudioClue(
            self,
            self.girl,
            ROOM_8_PLANK_HIT_SOUND,
            500,
            False,
    ...
    def play_room(self):
        ...
        if (
            ...
            and pygame.mouse.get_pressed()[0]
            and self.mother.show_fader == True
        ):
