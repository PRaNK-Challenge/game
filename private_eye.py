...
from clickable_classes import ADVENTURE_FONT, ADVENTURE_FONT_NON_ITALIC, NEXT_BUTTON, redraw_window, fade_in_and_out, change_cursor, Text, Clickable, FadeIn, AudioClue, CollectableClue, DraggableClue, ZoomableClue
...
ROOM_1_PHONE = pygame.transform.scale(
    pygame.image.load(os.path.join("art", "room1", "room1_phone.png")), (105, 90)
)
...
ROOM_1_PHONE_SOUND = pygame.mixer.Sound(os.path.join("sounds", "room_1_phone_ringing.mp3"))
ROOM_1_PHONE_PICKUP = pygame.mixer.Sound(os.path.join("sounds", "room_1_phone_pick_up.mp3"))
ROOM_1_DOOR_KNOCK = pygame.mixer.Sound(os.path.join("sounds", "room_1_door_knock.mp3"))
ROOM_1_CRYING = pygame.mixer.Sound(os.path.join("sounds", "room1_crying.mp3"))
ROOM_1_DRAWER = pygame.mixer.Sound(os.path.join("sounds", "room_1_drawer_sound.mp3"))
...
ROOM_6_VIP_MAN_END = pygame.transform.scale(
    pygame.image.load(os.path.join("art", "room6", "room6_vip_man_end.png")), (79, 248)
)
...
ROOM_7_FILE = pygame.transform.scale(
    pygame.image.load(os.path.join("art", "room7", "room3_file.png")), (95, 95)
)
...
# You lose sounds
YOU_LOSE_SAD = pygame.mixer.Sound(os.path.join("sounds", 'you_lose_sad.mp3'))
...
ROOM_8_MOTHER_HIT = pygame.transform.scale(
    pygame.image.load(os.path.join("art", "room8", "mother_hit8.png")), (350, 563)
)
...
ROOM_8_KEY_LOCK = pygame.mixer.Sound(os.path.join("sounds", "room8_key_lock.mp3"))
ROOM_8_GIRL_SPEAK = pygame.mixer.Sound(os.path.join("sounds", "room8_girl.mp3"))
ROOM_8_GIRL_SPEAK2 = pygame.mixer.Sound(os.path.join("sounds", "room8_girl2.mp3"))
ROOM_8_MOTHER_SPEAK = pygame.mixer.Sound(os.path.join("sounds", "room8_mother.mp3"))
ROOM_8_PLANK_HIT_SOUND = pygame.mixer.Sound(os.path.join("sounds", "room8_hit.mp3"))
ROOM_8_WOOD_SNAP = pygame.mixer.Sound(os.path.join("sounds", "room8_wood_snap.mp3"))

# Ending images
END = pygame.transform.scale(pygame.image.load(os.path.join("art", "goodend",'good.png')), (WIDTH, HEIGHT))
# Ending Sounds
END_SUCCESS = pygame.mixer.Sound(os.path.join("sounds", 'end_success.wav'))
...
class Room1():
    def __init__(self):        
        ...
        self.phone = Clickable(self, 630, 300, ROOM_1_PHONE)
        ...
        self.drawer_open = AudioClue(self, self.phone, ROOM_1_DRAWER, 100, False)
        self.key = DraggableClue(self, ROOM_1_KEY, 380, 385, True, area=self.drawer)
        self.text = Text("She seems too upset to speak. Maybe I should give her some tissues and she'll be able to tell us about the case?", "top", True)
    ...
    def play_room(self):
        ...
        if self.tissues.clicked == True:
            ...
            self.mother_speech.play_sound()
            self.mother_speech.sound = ""
        
        # key is revealed, can be moved
        if self.phone.clicked == True and self.tissues.clicked == True:
            self.text = Text("Oh good, that seems to have helped her. Now we can hear more about the case. And ah! There's the key to the drawer! Let's get that casefile.", "top", True)
            self.key.draw()
        ...
        if self.key.rect.colliderect(self.drawer):
            self.drawer_open.play_sound()
            self.drawer_open.sound = ""
...
class Room2():
    def __init__(self):
        ...
        self.diary2 = CollectableClue(self, 38, 365, ROOM_2_DIARY, 15, HEIGHT - 100)
        ...
        self.text = Text("Okay, where to look first? Hmm, those posters seems quite interesting. Oh! I've heard of magic ink before! You hold the note over heat and the ink becomes visible!", "top", True)
        self.text2 = Text("Aha, that looks like a diary key! Let's grab it and her diary and see what we can find out", "top", True)
    ...
    def play_room(self):
        ...
        if self.twinkle.image == self.key:
            self.text2.blit_text()
            self.diary2.self_vis = True
            self.diary2.draw()
            self.diary2.collect()
        
        # take the key to the diary, put both in inventory
        if self.diary2.clicked == True and self.twinkle.image == self.key:
...
class Room23():
    ...
    def play_room(self):
        ...
        display_text = ADVENTURE_FONT.render("Got a clue!", 1, WHITE)
...
class Room3():
    def __init__(self):
        ...
        self.photo = CollectableClue(self, 70, 140, ROOM_3_PHOTO, 700, 255)
        ...
        self.drawer_open = AudioClue(self, self.people, ROOM_1_DRAWER, 100, False)
        ...
        self.text = Text('"Do you know anything about where your friend Sara is? Her diary suggests you must know something"', "bottom", False)
    ...
    def play_room(self):
        ...
        if self.people.rect.collidepoint(pos) and pygame.mouse.get_pressed()[0]:
            self.peoplespeak.play_sound()
            self.peoplespeak.sound = ""
            
        if self.peoplespeak.sound != "":
            self.candle.clicked = False
            
        if self.peoplespeak.sound == "":
            self.text = Text("Hmm, well they won't be much help. What did the daughter, Emma, shove in the drawer just before I came in? It's locked, maybe the key card is around here...", "top", True)
        ...
        if self.photo.rect.topleft == (self.photo.next_x, self.photo.next_y):
            self.text = Text('"So are you sure you do not know anything at all? Who is this boy with you in the photo? And what is that file your father has?"', "bottom", False)
            self.daughterspeak.play_sound()
        ...
        if self.file.rect.topleft == (self.file.next_x, self.file.next_y):
            self.file.collect()
            self.text = Text("It seems the father collected information on this boy, Tom, as he was concerned too! Let's see if we can find that boy near that alleyway Emma mentioned.", "top", True)
...
class Room4():
    def __init__(self):
        ...
        self.water_run = AudioClue(self, self.drunk, ROOM_4_WATER_RUN, 500, False)
        ...
        self.text = Text("Well, this boy, Tom, doesn't seem to be anywhere around here anywhere. I wonder if this man has seen him...", "top", True)
    ...
    def play_room(self):
        ...
        if self.bottle.rect.colliderect(self.drunk_head) and self.bottle.image == self.full_bottle:
            ...
            self.drunk_talk2.play_sound()
            self.drunk_talk2.sound = ""
        
        if self.drunk_talk2.sound != "":
            self.bag.clicked = False
        ...
        if self.bag.image == self.open_bag and self.drunk_talk2.sound == "":
            ...
            self.necklace.draw()
            self.text = Text("This looks like a lady's necklace, could it be the one Tom gave to Sara? And on the back there's an address. Why would he try to get rid of it, maybe they had a fight?", "top", True)
        ...
        if self.necklace.clicked == True and self.drunk_talk2.sound == "":
            ...
            self.text = Text("Let's go check out that address and see if he will be there.", "top", True)
...
class Room5():
    def __init__(self):
        ...
        self.key = DraggableClue(self, ROOM_5_KEY, 720, 460, True, area= self.keyhole)
        self.key_end = Clickable(self, 720, 460, ROOM_5_KEY)
        ...
        self.text = Text("Looks like there's no one home. Nothing wrong with having a little look around though. Might be worth trying to get some clues before anyone comes home.", "top", True)
        self.start_items = [self.key, self.plant, self.lock, self.rock, self.keyhole]
        self.end_items = [self.plant, self.keyhole, self.poster, self.poster.next_room_button]
    ...
    def play_room(self):
        ...
        if self.key.rect.colliderect(self.keyhole):
            self.key = Clickable(self, 720, 460, ROOM_5_KEY)
            self.key.image = ""
            self.key.self_vis = False
            self.car.play_sound()
            self.car.sound = ""
            self.man_speak.play_sound()
        ...
        if self.man_speak.sound == "":
            self.text = Text("Looks like I'll need some way of breaking in to the garage! That lock doesn't look sturdy, with a little force it should open.", "top", True)
        ...
        if self.rock.rect.colliderect(self.lock):
            ...
            self.rock.image = ""
            self.rock.self_vis = False
            self.smash.play_sound()
            self.smash.sound = ""
            self.lock.image = ""
            self.text.remove_text()
        
        if self.rock.image == "" and self.rock.self_vis == False:
            self.rock.draw()
        ...
        if self.poster.self_vis == True:
            self.text = Text('"Hmm, poster advertising a club... And today the members are meeting up. Guess I am gonna go clubbing tonight..."', "bottom", False)
        
        if self.poster.clicked == True:
            self.poster.collect()
            self.rock.rect.topleft = (640, 405)
...
class Room6():
    def __init__(self):
        ...
        self.text = Text('"Hey, the bouncer won\'t let me through? Those guys are my friends from the club. Do you have a spare VIP ticket by any chance?"', "bottom", False)
        self.next_room_button = Clickable(self, 815, 490, NEXT_BUTTON)
    ...
    def play_room(self):
        ...
        if self.vip_man.clicked == True: 
            self.vip_man_speak.play_sound()
            self.vip_man_speak.sound = ""
            self.text.text = '"OK I guess I will be spending some money tonight..."'
...
class Room7():
    def __init__(self):
        ...
        self.poster_zoom = CollectableClue(self, 290, 20, ROOM_7_POSTER_ZOOM, 0, 0)
        self.file = Clickable(self, 196, 386, ROOM_7_FILE)
        self.file_zoom = CollectableClue(self, 260, 10, ROOM_7_FILE_ZOOM, 0, 0)
        self.necklace_text = Text('"Tom gave her a necklace with his address on it, who knows why he\'d want her to go there. But why would he throw it away?"', "bottom", False)
        self.file_text = Text("\"Her friend's father was gathering information on Tom, but seems like he didn't actually find a lot to be worried about, he's just a kid.\"", "bottom", False)
        self.photo_text = Text('"So, we know she was friends with Tom, but all the parents thought he was trouble."', "bottom", False)
        self.poster_text = Text("\"We didn't find him at the club with his gang, which could be a good sign. But it seems he ran with a rough crowd, I wouldn't trust them.\"", "bottom", False)
        self.diary_text = Text('"We know from her diary her mother was overprotective of her and would do anything to keep her away from Tom."', "bottom", False)
        self.final_text = Text('"Now that I\'m looking at all of this, I think I have a good idea of who might know where Sara is!"', "bottom", False)
        self.text = Text("", "bottom", False)
    ...
    def play_room(self):
        ...
        if final:
            ...
            if self.folder.clicked == True:    
                ...
                self.guess4.draw()
                self.guess1.draw()
                guess1 = ADVENTURE_FONT_NON_ITALIC.render(
                            "The Boy", 1, BLACK)
                WIN.blit(guess1, (250, 90))
                guess2 = ADVENTURE_FONT_NON_ITALIC.render(
                            "The Boy's Friends", 1, BLACK)
                WIN.blit(guess2, (550, 90))
                guess3 = ADVENTURE_FONT_NON_ITALIC.render(
                            "The Girl's Mother", 1, BLACK)
                WIN.blit(guess3, (210, 230))
                guess4 = ADVENTURE_FONT_NON_ITALIC.render(
                            "The Friend's Parents", 1, BLACK)
                WIN.blit(guess4, (550, 230))
...
class Room8:
    def __init__(self):
        ...
        self.stick1 = DraggableClue(self, ROOM_8_STICK1, 380, 503, True, area=self.plank2)
        self.stick2 = DraggableClue(self, ROOM_8_STICK2, 797, 490, True, area=self.plank1)
        ...
        self.tree = Clickable(self, 239, 179, ROOM_8_TREE)
        self.grass1 = Clickable(self, 630, 530, ROOM_8_GRASS1)
        self.grass2 = Clickable(self, 380, 540, ROOM_8_GRASS2)
        self.plank_hit = DraggableClue(self, ROOM_8_PLANK1, 506, 482, True)
        ...
        self.girl_speak = AudioClue(self, self.girl, ROOM_8_GIRL_SPEAK, 4500, False, func=self.mother.fade_in, func_pause=True)
        ...
        self.text = Text("So her mother wanted to keep her away from Tom and frame him for the kidnapping! The notes said Sara's mother owns this land, and someone seems to be inside that shed! I need to hurry!", "top", True)
        self.start_items = [self.plank1, self.plank2, self.plank3, self.stick1, self.stick2, self.stick3, self.lock, self.grass1, self.grass2, self.tree]
    ...
    def play_room(self):
        ...
        if self.stick1.area.rect.collidepoint(pos) and self.stick1.clicked == True:
            if (self.stick1.rect.colliderect(self.plank2) and pygame.mouse.get_pressed()[0]):
                self.wood_snap.play_sound()
                self.wood_snap.sound == ""                
                self.plank2.image = ""
                self.plank2.self_vis = False
                self.text.remove_text()
                self.stick1.rect.topleft = (0,0)
                self.stick1.image = ""
        ...
        if self.stick3.area.rect.collidepoint(pos) and self.stick3.clicked == True:
            if (self.stick3.rect.colliderect(self.plank3) and pygame.mouse.get_pressed()[0]):
                self.wood_snap = AudioClue(self, self.tree, ROOM_8_WOOD_SNAP, 50, False)
                self.wood_snap.play_sound()
                self.wood_snap.sound = ""
                self.stick3.rect.topleft = (0,0)
                self.plank3.image = ""
        ...
        if self.stick2.area.rect.collidepoint(pos) and self.stick2.clicked == True:
            if (self.stick2.rect.colliderect(self.plank1) and pygame.mouse.get_pressed()[0]):
                self.wood_snap = AudioClue(self, self.tree, ROOM_8_WOOD_SNAP, 50, False)
                self.wood_snap.play_sound()
                self.wood_snap.sound = ""
                self.stick2.rect.topleft = (0,0)
                self.plank1.image = ""
        ...
        if self.plank1.image == "" and self.plank1.self_vis == False:
            ...
            if self.mother.show_fader == True:
                ...
                WIN.blit(self.plank_hit.image, (506, 482))
        ...
        if self.plank_hit.rect.colliderect(self.mother_head) and pygame.mouse.get_pressed()[0] and self.mother.show_fader == True:
            ...
            self.mother.rect.topleft = (135, 180)
            self.plank_hit_mother.play_sound()
...
# decide which room to play and how to move between rooms
class GameState():
