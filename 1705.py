...
PHONE = pygame.transform.scale(pygame.image.load('room1_phone.png'), (130,135))
...
MOTHER = pygame.transform.scale(pygame.image.load('room1_mother.png'), (250,320))
...
# Room 1 sounds
WOMAN = pygame.mixer.Sound('room_1_woman.mp3')
LITTLE = pygame.mixer.Sound('little_girl_voice.mp3')
MAN = pygame.mixer.Sound('room_1_man.mp3')
...
ROOM_2_KEY = pygame.transform.scale(pygame.image.load('room2_key.png'), (20, 20)).convert_alpha()
ROOM_2_DIARY = pygame.transform.scale(pygame.image.load('room2_diary.png'), (100, 70)).convert_alpha()
ROOM_2_DIARY_ZOOM = pygame.transform.scale(pygame.image.load('room2_diary_zoom.png'), (700, 550)).convert_alpha()
...
class Room1():
    def __init__(self):        
        ...
        self.text = Text("She's too upset to speak. Maybe give her the tissues and she'll explain?")
        self.mother = FadeIn(self, 105, 120, MOTHER)
        self.phone = Clickable(self, 620, 290, PHONE)
        self.phone_call = AudioClue(self, self.phone, PHONE_PICKUP, 1500, MAN, DOOR_KNOCK, self.mother.fade_in)
        ...
        self.items = [self.drag_key, self.phone, self.mother, self.click_tissues, self.collect_folder]
...
class Room2():
    def __init__(self):
        ...
        self.next_room = False
        self.items = [self.candle, self.diary, self.open_diary, self.note, self.note_zoom, self.twinkle, self.key]
    
    def play_room(self):
        WIN.blit(self.image, (0,0))
        pos = pygame.mouse.get_pos()
        
        clicked_items = [item for item in self.items if item is not self and isinstance(item, DraggableClue) and item.clicked == True]
        ...
        if self.diary.clicked == True:
            ...
            if pygame.mouse.get_pressed()[0] and not self.diary.clue_space.collidepoint(pos):  
        ...
        if self.candle.clicked == True:
            if self.note.is_zoom == True:
                ...
                if pygame.mouse.get_pressed()[0] and not self.candle.clue_space.collidepoint(pos) and len(clicked_items) == 0:  
                    self.candle.toggle_image = ""
            
        if pygame.mouse.get_pressed()[0] and not self.candle.clue_space.collidepoint(pos):  
            self.candle.toggle_image = ""
